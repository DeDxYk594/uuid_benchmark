...
func Select() int64 {
	...
	rows, err := db.Query(`SELECT u.u_id, f.file_uuid FROM "user" AS u JOIN "file" AS f ON u.avatar_file_uuid = f.file_uuid WHERE u.u_id=$1 LIMIT 1;`, u_id)
	if err != nil {
		log.Fatal(err)
	}
	rows.Close()
...
func Insert() int64 {
	...
	var fileUUID string
	err := db.QueryRow(`INSERT INTO "file"(file_extension, created_at) VALUES ($1, CURRENT_TIMESTAMP) RETURNING file_uuid`,
		"jpg").Scan(&fileUUID)
	if err != nil {
		log.Fatal(err)
	}
	...
	_, err = db.Exec(`INSERT INTO "user"(u_id, avatar_file_uuid) VALUES ($1, $2)`, uid, fileUUID)
	if err != nil {
		log.Fatal(err)
	}
	insertDuration = time.Since(start)
